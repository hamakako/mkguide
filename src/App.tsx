...
import { ArrowLeft, ArrowUpLeft, CalendarDays, Check, ChevronDown, Clock3, Compass, Download, Headphones, Heart, Map, MapPin, Menu, MessageCircle, Search, ShieldCheck, Sparkles, Star, Users, Utensils, X } from 'lucide-react'
...
function CityPage() {
  ...
  return <main className="city-page">
    ...
    <section className="section"><div className="container"><SectionHead kicker="نابێت لەدەستت بچێت" title={`شوێنە گرنگەکانی ${city.name}`} text={`${kn(city.attractions.length)} شوێنی ڕاستەقینە، هەر یەک بە وێنە، زانیاری و لینکی نەخشە.`}/><div className="attraction-grid">{city.attractions.map((a,i)=><article className="attraction" key={a.name}><Link className="attraction-hit" to={`/city/${city.slug}/attraction/${a.slug}`} aria-label={`زانیاریی زیاتر دەربارەی ${a.name}`}/><div><BrandedImage src={a.image} alt={a.name}/><span className="number">{kn(i+1)}</span><a className="photo-credit" href={a.photoSource} target="_blank" rel="noreferrer">Wikimedia</a></div><section><h3>{a.name}</h3><p>{a.description}</p><span className="more-link">زانیاریی زیاتر <ArrowLeft/></span><footer><span><Clock3/> {a.duration}</span><a href={a.map} target="_blank" rel="noreferrer"><MapPin/> لە نەخشە</a></footer></section></article>)}</div></div></section>
    <section className="section itinerary"><div className="container"><SectionHead kicker="ڕۆژ بە ڕۆژ" title="پلانی گەشتەکەت" text="کاتەکان بە ئارامی ڕێکخراون؛ دەتوانیت بە پێی حەزت بیانگۆڕیت."/><div className="plan-actions"><div className="day-tabs">{[3,5,7].map(d=><button className={days===d?'active':''} onClick={()=>setDays(d)} key={d}>{kn(d)} ڕۆژ</button>)}</div><a className="pdf-download" href={`/pdfs/${city.slug}-daily-guide.pdf`} download><Download/> داگرتنی پلانی PDF</a></div><div className="timeline">{plans.map((p)=><article className="day" key={p.day}><div className="day-marker"><span>{kn(p.day)}</span><i/></div><div className="day-card"><div className="day-photo"><BrandedImage src={p.image} alt={`ڕۆژی ${kn(p.day)}`}/></div><div className="day-copy"><small>ڕۆژی {kn(p.day)}</small><h3>{p.title}</h3><p>{p.morning}</p><p>{p.afternoon}</p><p>{p.evening}</p><div className="day-meta"><span><Clock3/> {p.duration}</span><span><Compass/> {p.transport}</span></div><div className="advice"><Sparkles/> {p.advice}</div><a href={p.map} target="_blank" rel="noreferrer">کردنەوە لە نەخشە <ArrowUpLeft/></a></div></div></article>)}</div></div></section>
...
function AttractionPage() {
  ...
  return <main className="attraction-page">
    <section className="attraction-detail-hero">
      ...
      <a className="detail-photo-credit" href={attraction.photoSource} target="_blank" rel="noreferrer">وێنە: {attraction.photoCredit} - {attraction.photoLicense}</a>
